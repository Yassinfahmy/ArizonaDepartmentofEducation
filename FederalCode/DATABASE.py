...
class DATABASE:
    ...
    def __init__(self,  fiscal_year=None, database='AccountabilityArchive', schema='Static', run='Prelim'):
        '''
    ...
    def drop_tables_in_run(self, *tables, table_prefix=None):
        ...
        for table in tables:
            table_name=F'{self.database}.{self.schema}.{table_prefix}{table}' + str(self.fiscal_year)
            ...
            try:
                ...
                print(f'Table \"{table_name}\" not found or could not be deleted')
                # traceback.print_exc() # this was printing too much for me to make sense of -Tobi
        cnxn.close()
    ...
    def upload_table_to_db (self, df, table_name, all_nvarchar=False):
        ...
        df = df.copy() # do not overwrite dataframe
        new_table_name =F'{self.database}.{self.schema}.{self.run}{table_name}' + str(self.fiscal_year)
    ...
    def fill_table(self, df:pd.DataFrame, table_name, clear_table=True, all_data=False, cols_to_ignore=None, 
        create_if_not_exist:bool=True, **kwargs):
        '''
        This is the right method to use from outside of class
        Parameters
        ----------
        ...
        if cols_to_ignore is None:
            cols_to_ignore = ['Key', 'CreatedBy', 'CreatedDate', 'LastModifiedBy', 'LastModifiedDate', 'KThru8', 'AThruF']
        # setup a connection to db
        cnxn =  self.connect_to_db()
        cursor = cnxn.cursor()
        #clear table if indicated
        full_table_name = f"{self.database}.{self.schema}.{self.run}{table_name}{self.fiscal_year}"
        if clear_table:
            try:
                if all_data is False:
                    ...
                    delete_cy = ''
                sql_clear = f'Delete {full_table_name} {delete_cy}'
                cursor.execute(sql_clear)
                print(colored(f'"{sql_clear}" ==> Executed Successfully', 'green'))
            except Exception as ex:
                print(ex)
                print(colored(f'Warning:\n"Delete {full_table_name} WHERE FiscalYear = {self.fiscal_year}" ==> Failed', 'red'))
                
        try:
            ## read empty table cols
            sql_read = f'SELECT top(1)* FROM {full_table_name}'
            target_table = pd.read_sql_query(sql_read, cnxn)
            print(colored(f'{full_table_name} ==> read Successfully', 'green'))
        except Exception as ex:
            print(colored(f'"{sql_read}" ==> Failed to read target table', 'red'))
            print(ex)
            print('******************************************\n')
            if create_if_not_exist:
                self.upload_table_to_db(df, table_name, **kwargs)
                print(f"Creating a new table called {full_table_name}.")
        else:
            ...
            if missing_cols.shape[1]>0:
                print(colored(f'****Warning****:\n{missing_cols.shape[1]} columns in "{full_table_name}" Not found in source DataFrame:', 'red'))
                print(*missing_cols.columns.to_list())
                print(colored('Columns will be filled with "."', 'red'))
                #recreate misssing cols as empty fields
            ...
            df = df.apply(lambda x: x.astype(float, errors="ignore"))
            numeric_only = df.select_dtypes("number")
            integer_columns = numeric_only.columns[(((numeric_only % 1)==0) | numeric_only.isna()).all(0)]
            for col in integer_columns:
                df[col] = df[col].astype("Int64")
            df = df.replace({pd.NA:"."})
            df.fillna('.', inplace=True)
            df = df.astype(str) # keep values rounded in SQL
            # upload the new table
            # table_name = f"{self.database}.{self.schema}.{self.run}{table_name}{self.fiscal_year}"
            self.upload_to_server(df, full_table_name, cnxn)
            
    def read_table(self, table_name:str, where_clause:str=""):
        '''
        Parameters
        ----------
        table_name : str
        ...
        sql = F'SELECT * FROM {new_table_name} {where_clause}'
        try:
            # setup a connection to db
            cnxn =  self.connect_to_db()
            #read data
    ...
    # 
    def get_ordered_columns(self, table_name):
        sql = f"""
            SELECT TOP (0) * FROM {self.database}.{self.schema}.{self.run}{table_name}{str(self.fiscal_year)} 
        """
        return self.read_sql_query(sql=sql).columns.to_series()
    

    """
    Checks that the columns from one dataframe (target) are all present in another (result) in a case-insensitive manner. Then extracts 
        the columns that are in target from result in the order they appear in target and returns them in a new dataframe.
    """
    def order_columns(self, result_table:pd.DataFrame, target_table:pd.DataFrame):
        result_table.columns = result_table.columns.str.lower()
        try: # attempt to extract all of the target columns from the results in the order that they appear in the target table
            result_table = result_table[target_table.columns.str.lower()] 
        except KeyError: # handle error if all of target columns are not present in the results
            columns_missing = [x for x in target_table.columns if str(x).lower() not in result_table.columns]
            raise KeyError(f"The following columns are missing from the \"result_table\": {columns_missing}")
        map = {str(x).lower():x for x in target_table.columns} # define map from lowercase name to target name
        result_table.columns = result_table.columns.to_series().replace(map) # replace lowercase names with target names
        return result_table